    ...
    <div class="card-primary">
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                ...
                <div class="card-body">
                    <form action="{{ route('services.update',[$service->id])}}" method="POST" id="formid" enctype=multipart/form-data>
                        @csrf
                        @method('patch')
                        <div class="form-group">
                            <label for="title">{{__('Name')}}</label>
                            <input type="text" class="form-control" placeholder="Name of person"
                                   name="name" id="name" required value="{{$service->name}}">
                        </div>
                        <div class="form-group">
...
{{--                            <img src="{{$image}}">--}}
                        ...
                        <div class="form-group">
                            <div class="radio">
                                <label><b>{{__('Status')}}</b> <br><input type="radio" name="status" {{$service->status == 1 ? 'checked' : ''}}
                                    value="1">{{__('Active')}}</label>

                                <label><input type="radio" name="status" {{$service->status == 0 ? 'checked' : ''}}
